...
interactions_df = pd.read_csv('out.csv')
...
interactions_full_df = interactions_from_selected_users_df \
    .groupby(['personId', 'contentId'])['rating'].sum() \
...
class ModelEvaluator:
    ...
    def get_not_interacted_items_sample(self, person_id, sample_size, seed=42):
        ...
        random.seed(seed)
        non_interacted_items_sample = random.sample(non_interacted_items, sample_size - 100)
...
item_popularity_df = interactions_full_df.groupby('contentId')['rating'].sum().sort_values(
...
print(len(user_profiles.keys()))
print("-----------------------------")
myprofile = user_profiles[7000]
print(myprofile.shape)
aa = pd.DataFrame(sorted(zip(tfidf_feature_names,
                             user_profiles[7000].flatten().tolist()), key=lambda x: -x[1])[:20],
...
aaa: pd.DataFrame = inspect_interactions(7000, test_set=False)
aaa.to_csv("testout.csv")
print()
